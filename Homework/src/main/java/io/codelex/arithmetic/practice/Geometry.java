...
class Geometry {
    ...
    static BigDecimal areaOfRectangle(BigDecimal length, BigDecimal width) {
        if (length.compareTo(BigDecimal.valueOf(0)) <= 0 || width.compareTo(BigDecimal.valueOf(0)) <= 0) {
            throw new IllegalArgumentException("Value cannot be less than 0!");
        }
    ...
    static BigDecimal areaOfTriangle(BigDecimal base, BigDecimal h) {
        if (base.compareTo(BigDecimal.valueOf(0)) <= 0 || h.compareTo(BigDecimal.valueOf(0)) <= 0) {
            throw new IllegalArgumentException("Value cannot be less than 0!");
        }
