...
public class NumberOperations {
    ...
    */
    ...
    public static void main(String[] args) {
        ...
        Runnable sum = () -> SumFromList(numberList);
        Runnable average = () -> ListAverage(numberList);
        Runnable sumSecond = () -> SumOfAllSecondNumbers(numberList);
        ExecutorService executor = Executors.newFixedThreadPool(3);
        executor.execute(sum);
        executor.execute(average);
        executor.execute(sumSecond);
        executor.shutdown();
    }
    ...
    public static void SumFromList(List<Integer> list) {
        int sum = list.stream().collect(Collectors.summingInt(Integer::intValue));
        System.out.println("Sum of all elements: " + sum);
    }

    public static void ListAverage(List<Integer> list) {
        double average = list.stream().collect(Collectors.summingInt(Integer::intValue)) / list.size();
        System.out.println("Average: " + average);
    }

    public static void SumOfAllSecondNumbers(List<Integer> list) {
        int sumOfSeconds = IntStream.range(0, list.size()).filter(i -> i % 2 != 0).
                mapToObj(i -> list.get(i)).collect(Collectors.toList())
                .stream().collect(Collectors.summingInt(Integer::intValue));
        System.out.println("Sum of all second numbers: " + sumOfSeconds);
    }
